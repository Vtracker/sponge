...
void TCPSender::fill_window() {
    ...
    while (curr_window_size > _outgoing_bytes) {
        ...
        _flyings_segs.push(make_pair(_next_seqno, segment));
        // 更新待发送 abs seqno
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (abs_seqno > _next_seqno)
        return;
    // 遍历数据结构，将已经接收到的数据包丢弃
    while (_flyings_segs.size()) {
        // 如果一个发送的数据包已经被成功接收
        auto x = _flyings_segs.front();
        const TCPSegment &seg = x.second;
        if (x.first + seg.length_in_sequence_space() <= abs_seqno) {
...
void TCPSender::tick(const size_t ms_since_last_tick) {
    ...
    auto x = _flyings_segs.front();
    // 如果存在发送中的数据包，并且定时器超时
    if (_flyings_segs.size() && _timecount >= _timeout) {
        // 如果窗口大小不为0还超时，则说明网络拥堵
        if (_last_window_size > 0)
            _timeout *= 2;
        _timecount = 0;
        _segments_out.push(x.second);
        // 连续重传计时器增加
